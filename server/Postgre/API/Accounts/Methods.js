...
async function getFirstAccountProtocolInfo(req, res, next) {
  try {
    const { rows } = await db.query(queries.query_getfirstaccountprotocolinfo(req));
    return rows;
  } catch (error) {
    next(error);
  }
...
async function getAccountById(contractId, accountId, accountNumber, next, client) {
  try {
    var poolQuery = client ? client : db;
...
async function getAccountsInfo(req, res, next) {
  try {
    const { rows } = await db.query(queries.query_getaccountsinfo(req));
    return rows;
  } catch (error) {
    next(error);
  }
...
async function getAccountInfo(req, res, next) {
  try {
    const { rows } = await db.query(queries.query_getaccountinfo(req));
    return rows;
  } catch (error) {
    next(error);
  }
...
async function updateAccount(req, res, next) {
  try {
    const { rows } = await db.query(queries.query_updateaccount(req));
    return rows;
  } catch (e) {
    throw e;
  }
