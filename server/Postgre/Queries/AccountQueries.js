...
const getAccountById =  (req, res, next, accountId) => {
  ...
  var sqlQuery = util.format('SELECT *, ' +
    ...
    '(SELECT json_agg(AccountReservations) FROM (SELECT * FROM "Ordering"."AccountReservations" as accres WHERE accres."AccountId" = a."Id" ORDER BY accres."Order") AccountReservations) AS AccountReservations, ' +
