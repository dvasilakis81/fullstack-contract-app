...
const insert = (req, res, next) => {
  ...
  pool.query(sqlQuery, (error, results) => {
    if (error)
      ...
      accountMethods.getAccountById(req, res, next, accountId);
  })
}
const sync = (req, res, next) => {
  var userId = req.body.userId;

  var sqlQuery = 'SELECT * FROM "Ordering"."AccountReservations" as ur WHERE ur."UserId"=' + helper.addQuotes(userId);
  pool.query(sqlQuery, (error, results) => {
    if (error)
      next(error);
    else {
      if (results && results.rows && results.rows.length > 0) {
        sqlQuery = 'DELETE FROM "Ordering"."AccountReservations" as ur WHERE ur."UserId"=' + helper.addQuotes(userId);
        pool.query(sqlQuery, (error, results) => {
          if (error)
            next(error);
          else
