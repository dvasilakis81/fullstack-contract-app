...
const processMonitoringCommittee = (req, res, next, accountInfo) => {
  ...
  ret = pool.query(sqlQuery, (error, results) => {
    if (error)
      ...
      if (results.rows.length === 0)
        insertMonitoringCommittee(req, res, next, accountInfo)
      else
        updateMonitoringCommittee(req, res, next, accountInfo)
    }
  })
}
